...
for line in f:
  ...
  if m:
    if scenario=='mode_norm.fast.RCmin_bc' and group=='clk':
      whs=m.group(1)
      break
f.close()
...
for line in f:
  ...
  if m:
    if scenario=='mode_norm.slow.RCmax' and group=='clk':
...
for line in f:
  ...
  m=re.search(r'\s*Layer\s*li1\s*:\s*(\d+)',line)
#  print(f'\n\n {m}')
  if m:
    li1_wire_len=m.group(1)
    print('######################################')
    print('New statistics [wirelength -- route_opt.check_routes]')
    print(f'route_opt__area__li1_wirelength__tot = {li1_wire_len}')
    continue
...
#  print(f'\n\n {m}')
  if m:
    ...
    print(f'route_opt__area__met1_wirelength__tot = {met1_wire_len}')
    continue

  m=re.search(r'\s*Layer\s*met2\s*:\s*(\d+)',line)
#  print(f'\n\n {m}')
  if m:
    met2_wire_len=m.group(1)
    print(f'route_opt__area__met2_wirelength__tot = {met2_wire_len}')
    continue

  m=re.search(r'\s*Layer\s*met3\s*:\s*(\d+)',line)
#  print(f'\n\n {m}')
  if m:
    met3_wire_len=m.group(1)
    print(f'route_opt__area__met3_wirelength__tot = {met3_wire_len}')
    continue

  m=re.search(r'\s*Layer\s*met4\s*:\s*(\d+)',line)
#  print(f'\n\n {m}')
  if m:
    met4_wire_len=m.group(1)
    print(f'route_opt__area__met4_wirelength__tot = {met4_wire_len}')
    continue

  m=re.search(r'\s*Layer\s*met5\s*:\s*(\d+)',line)
#  print(f'\n\n {m}')
  if m:
    met5_wire_len=m.group(1)
    print(f'route_opt__area__met5_wirelength__tot = {met5_wire_len}')
    print('######################################\n\n')
    break
...
# route_opt__area__li1_wirelength__tot
results.write(li1_wire_len+',')
# route_opt__area__met1_wirelength__tot
results.write(met1_wire_len+',')
# route_opt__area__met2_wirelength__tot
results.write(met2_wire_len+',')
# route_opt__area__met3_wirelength__tot
results.write(met3_wire_len+',')
# route_opt__area__met4_wirelength__tot
results.write(met4_wire_len+',')
# route_opt__area__met5_wirelength__tot
results.write(met5_wire_len+',')

f=open('icc2rm/rpts_icc2/route_opt.report_power')
for line in f:
    m = re.search(r"^\s*Cell Internal Power\s*=\s*([\d.e+-]+)\s*nW", line)
    if m:
        power_int=m.group(1)
        print('######################################')
        print('New statistics [power --  route_opt.report_power] ')

        print(f'route_opt__power__int__tot = {power_int}')
        continue
    m = re.search(r"^\s*Net Switching Power\s*=\s*([\d.e+-]+)\s*nW", line)
    if m:
        power_sw=m.group(1)
        print(f'route_opt__power__sw__tot = {power_sw}')
        continue
    m = re.search(r"^\s*Cell Leakage Power\s*=\s*([\d.e+-]+)\s*nW", line)
    if m:
        power_leak=m.group(1)
        print(f'route_opt__power__leak__tot = {power_leak}')
        print('######################################')
        break

f.close()

# route_opt__power__int__tot
results.write(power_int+',')
# route_opt__power__sw__tot
results.write(power_sw+',')
# route_opt__power__leak__tot
results.write(power_leak+'\n')
