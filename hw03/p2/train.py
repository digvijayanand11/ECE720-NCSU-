...
# crossvalIter = KFold(n_splits=10,shuffle=True, random_state=random_seed)
...
# data_lengths = { 'train':50, 'valid':450, 'test':len(testDS) }
...
for split, (trainID,validID) in enumerate(crossvalIter.split(crossvalDS)):
    ...
    # Initialize Model
    # model = MyModel(in_dim=len(fullDS.features),out_dim=len(fullDS.targets),layers=[64]).to(device)
    model = MyModel(in_dim=len(fullDS.features),out_dim=len(fullDS.targets),layers=[64]).to(device)
    model.resetWeights()
    if os.access(f'mymodel{split}.pt',os.F_OK):
        model.load_state_dict(torch.load(f'mymodel{split}.pt'))
    model.eval()
    # print(model)

    # Choose loss function and optimizer
    loss_fn = torch.nn.MSELoss()
    optimizer = torch.optim.AdamW(model.parameters(),lr=1e-3,weight_decay=2)
    ...
    if epochs > 0:
        ...
        for t in range(epochs):
            ...
            if t==epochs-1:          # Use this line to save the last epoch's model
                torch.save(model.state_dict(),f'mymodel{split}.pt')
        ...
        plt.savefig(f'train-loss{split}.png')
        plt.close()
...
plt.xlabel('APE')
plt.savefig('basemodel-error-summary.png',bbox_inches='tight')
# plt.show()
plt.close()

os.system('date +%s > train')
